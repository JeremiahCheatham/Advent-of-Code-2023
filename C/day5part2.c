...
void convert_seeds(struct Data *data) {
    for (int i = 0; data->seeds[i]; i += 2) {
        unsigned int seed_start = data->seeds[i];
        unsigned int seed_range = data->seeds[i + 1];
        for (unsigned int seed = seed_start; seed < seed_start + seed_range; seed++) {
